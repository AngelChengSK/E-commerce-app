...
// import {masterProductList} from '../server/data.js'
...
const app = {
  ...
  getMasterProductList: () => {
    return fetch('http://localhost:3000/get-master-product-list', {
      method: 'POST',
      headers: {
  ...
  renderNewArrivals: async () => {
    const masterProductList = await app.getMasterProductList()
  ...
  showSearchResult: async (searchPhase) => {
    const masterProductList = await app.getMasterProductList()
  ...
  renderSelectedCategory: async(categoryName) => {
    const masterProductList = await app.getMasterProductList()
